...
from bcos.common import plot_contribution_map
...
class LocalisationAnalyser(Analyser):
    ...
    def __init__(
        ...
        **config,
        ...
        # do not reload for none
        if self.config["reload"] != "none":
    ...
    def analysis(self) -> Dict[str, Any]:
        ...
        metric = []
        # Track how many pixels LIME includes...
        num_pixels = []
        ...
        for count in tqdm(range(sample_size), desc="Analysis..."):
            ...
            if self.config["striding"] != "none":
                ...
                patches = torch.nn.functional.unfold(
                    multi_img, ks, stride=stride, padding=padding
                ).view(*multi_img.shape[:2], ks, ks, -1)
                att_list = []
                for patch_idx in range(patches.shape[-1]):
                    att_list.append(
                        explainer.attribute_selection(
                            patches[..., patch_idx], tgts
                        ).sum(1, keepdim=True)[..., None]
                    )

                attributions = torch.nn.functional.fold(
                    torch.cat(att_list, dim=-1).view(
                        len(multi_img) * len(tgts), -1, patches.shape[-1]
                    ),
                    multi_img.shape[-2:],
                    stride=stride,
                    kernel_size=ks,
            ...
            if explainer.__class__.__name__ == "Lime":
                num_pixels.append(
                    [att.detach().sum().cpu().numpy() for att in attributions]
                )
            
            # START <<<<<<<<<<<<<<<< Plotting >>>>>>>>>>>>>>>>>>
            
            # Plot every multiple of 50
            if count % 50 == 0:
                # ------------ Plotting original image grid ------------
                rgb_img = multi_img[0, :3, :, :]

                # Convert to numpy array and transpose to channel-last format for plotting
                rgb_img = rgb_img.permute(1, 2, 0).cpu().detach().numpy()

                # Plotting
                plt.imshow(rgb_img)
                plt.axis('off')  # Optionally turn off the axis
                
                path_to_save = str(self.save_folder / f"original_img_{count}.png")

                # Saving the plot
                plt.savefig(path_to_save, bbox_inches='tight')
                plt.close()
                
                # ------------ Plotting attribution grids ------------
                # Create a figure with a 3x3 grid of subplots
                
                # grid_size = n_imgs // 2
                grid_size = int(np.ceil(np.sqrt(n_imgs))) # New: Fix extra plots in Conv loc plotting
                fig, axes = plt.subplots(grid_size, grid_size, figsize=(12, 12))
                
                # ---------------- function call version ---------------- #
                # Loop through all the attribution maps
                for i in range(n_imgs):
                    # ax = axes[i // 3, i % 3] # column-wise
                    ax = axes[i % grid_size, i // grid_size] # row-wise
                    # ax = axes[i // grid_size, i % grid_size]  # row-wise
                    attr = attributions[i].squeeze()
                    label = tgts[i]

                    # Use the plot_contribution_map function for each attribution map
                    plot_contribution_map(attr, ax=ax, cmap='bwr', percentile=99.5, class_index=label)
                # ---------------- function call version ---------------- #

                # New: Fix extra plots in Conv loc plotting
                # Hide any unused subplots
                for j in range(n_imgs, grid_size * grid_size):
                    fig.delaxes(axes[j // grid_size, j % grid_size])

                # Adjust layout
                plt.tight_layout()
                path_to_save = str(self.save_folder / f"attribution_grid_{count}.png")
                # Save the figure
                plt.savefig(path_to_save, bbox_inches='tight')
            ...
            if count % 50 == 0:
                print(f"Mean localisation metric: {np.array(metric).flatten().mean()}, Number of data points: {len(np.array(metric).flatten())}")

        result = np.array(metric).flatten()
        print(f"Mean localisation metric: {np.mean(result)}, Number of data points: {len(result)}")
        if self.config["neg"]:
            result = 1 - result
        print()
        print(
        ...
        print()
        return {
            "localisation_metric": result,
            **({"num_pixels": np.array(num_pixels).flatten()} if num_pixels else {}),
        }
...
def argument_parser(multiple_args=False, add_help=True):
    ...
    parser.add_argument(
        ...
        default="last",
        type=str,
        help="Which epoch to load. Options are 'last', 'best', 'best_any' and 'epoch_X', 'none'"
    ...
    parser.add_argument(
        ...
        default="none",
        type=str,
        help="Whether or not to use the model as a non-linear kernel.",
    )
    parser.add_argument(
        "--explainer_config",
        default="default",
